...
/**
 * This class holds an object that is specific to the Yokogawa DX200 recorder.
 * It contains procedures that are specific to reading the DX200 configuration
 * file and communicating with the DX200 recorder.
 * 
 * @author Scott Arneson
 */
public class YokogawaDX200 extends RecorderAbstract implements RecorderInterface {

    /**
     * Creates a YokogawaDX200 object. Sets various parameters needed for the
     ...
     */
    public YokogawaDX200(String modelNumber, String configFile, String ipAddress, String unitID, String priority) {
    ...
    public void parseConfiguration(List<String> lines) {
        ...
        for (String line : lines) {
            ...
            if (data.length > 1) {
                ...
                if (data[0].startsWith("SR")) {
                    numberOfDataPoints++;
                    if (data[1].contains("SKIP") || data[1].contains("VOLT,2V,-2000,2000")) {
                        this.addUnit("UNUSED");
                        this.addDecimal(0);
                    } else if (data[1].contains("DELTA")) {
                        if (data[1].contains("VOLT")) {
                            this.addUnit("NO UNITS");
                            if (data[1].contains("200MV")) {
                                this.addDecimal(1);
                            } else if (data[1].contains("2V") || data[1].contains("6V")) {
                                this.addDecimal(3);
                            } else {
                                this.addDecimal(2);
                            }
                            ...
                            this.addUnit("F");
                            if (configValues[2].equalsIgnoreCase("E") || configValues[2].equalsIgnoreCase("J")
                                    || configValues[2].equalsIgnoreCase("L") || configValues[2].equalsIgnoreCase("U")
                                    || configValues[2].equalsIgnoreCase("T")) {
                                this.addDecimal(1);
                            } else {
                                this.addDecimal(0);
                            }
                    ...
                    } else if (data[1].contains("SQRT") || data[1].contains("LOG")) {
                        this.addUnit(configValues[configValues.length - 1].trim());
                        this.addDecimal(Integer.parseInt(configValues[configValues.length - 2]));
                ...
                //Lines starting with "SO" indicate math channels setup
                if (data[0].startsWith("SO")) {
                    numberOfMathPoints++;
                    if (configValues[0].equalsIgnoreCase("ON")) {
                        this.addUnit(configValues[configValues.length - 1].trim());
                        this.addDecimal(Integer.parseInt(configValues[configValues.length - 2]));
                    } else {
                        this.addUnit("UNUSED");
        ...
        //Data register addresses and sizes come from tech manual section 4.2
        this.setPort(4660);
        this.setNumberOfDataPoints(numberOfDataPoints);
        this.setNumberOfMathPoints(numberOfMathPoints);
        this.setStartDataRegisterAddress(0);            //Reference for register 30001
        this.setDataRegisterMultiplicationConstant(1);
        this.setStartAlarmsRegisterAddress(1000);       //Reference for register 31001
        this.setStartMathRegisterAddress(2000);         //Reference for register 32001
        this.setMathRegisterMultiplicationConstant(2);
        this.setStartMathStatusRegisterAddress(3000);   //Reference for register 33001
        this._count = 9;
    }
    ...
    public void getData() {
        ...
        if (this.getModbusConnection().isOpen()) {
            ...
            int[] mathData = ReceiveEthernetStrategy.getInstance().getBigEndianData(this,
            ...
            if (mathData.length > 0) {
                //Evaluate the math data which is 32bit stored ins 2 16bit registers big endian style (Section 4.2 of tech manual)
            ...
            //Alarm information is given in sections 4.2 and 6.3(page 6-21) of the tech manual
            if (alarms.length > 0) {
